...
class EventCollectionViewCell: UICollectionViewCell {
    ...
    func configure(for event: Event){
        ...
        eventTime.text = ""
        let timeArray = event.eventTime
        let timeSize = timeArray?.count
        if (timeSize! > 0){
            for index in 0..<(timeSize! - 1){
                eventTime.text = eventTime.text! + timeArray![index] + ", "
            }
            eventTime.text = eventTime.text! + timeArray![-1]
        }
        
        eventLocation.text = ""
        let locationArray = event.eventLocation
        let locationSize = locationArray?.count
        if (locationSize! > 0){
            for index in 0..<(locationSize! - 1){
                eventLocation.text = eventLocation.text! + locationArray![index] + ", "
            }
            eventLocation.text = eventLocation.text! + locationArray![-1]
        }
